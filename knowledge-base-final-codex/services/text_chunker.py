...
def chunk_document(parsed_doc, chunk_size=None, overlap=None):
    import config
    ...
    for i, text in enumerate(chunks):
        ...
        if parsed_doc.pages and len(parsed_doc.pages) > 1:
            best_page = None
            best_score = 0
            query_start = text[:200]
            query_ngrams = set(query_start[i:i+3] for i in range(len(query_start) - 2))
            for pi, pt in enumerate(parsed_doc.pages):
                page_ngrams = set(pt[i:i+3] for i in range(len(pt) - 2))
                if not query_ngrams:
                    continue
                score = len(query_ngrams & page_ngrams) / len(query_ngrams)
