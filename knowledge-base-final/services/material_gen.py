...
def generate_material(framework_text, api_key=DEEPSEEK_API_KEY):
    if not api_key:
    ...
    for i, section in enumerate(sections):
        ...
        try:
            search_results = index.search(query, api_key, TOP_K_RESULTS)
        except Exception:
            search_results = index._keyword_fallback(query, TOP_K_RESULTS)
...
5. 字数不少于 300 字"""
        ...
        content = _call_chat_api(prompt, api_key)
        if not content:
    ...
    polished = _call_chat_api(polish_prompt, api_key)
...
def _call_chat_api(prompt, api_key, temperature=0.5):
    retries = 3
    for attempt in range(retries):
        try:
            data = json.dumps({
                "model": CHAT_MODEL,
                "messages": [
                    {"role": "system", "content": "你是一个专业的文档编写助手，擅长根据大纲和参考资料撰写审核材料。"},
